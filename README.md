...
일간 `candidates.py generate`(cards 단계)는 스크리너의 A·B 상위 목록을 번갈아 놓은 순서(A1·B1·A2…, 기업당 1회, 투자 점수 아님)로 후보 카드를 만든다. 후보 ID는 `CAN-`+SHA256(기업 식별자|가설 키) 앞 16자리이며, 기업 식별자는 entities.json 등록값, 없으면 SEC 목록의 거래소:티커다(CIK 추측 없음, 거래소를 모르면 ID·추적 명령 없이 표시). 가설 키는 `eps-revision-review:v1`(EPS 상향의 사업 원인과 지속성 검토). 후보 버전은 카드 주장(식별·EPS 수치·대상 기간·목록 충족·분류 입력·설명·출처)의 해시로, 수집 시각·순위·매일 움직이는 주가 구간은 버전을 바꾸지 않는다. 버전은 `data/processed/candidate_history/{버전}.json`에 한 번만 저장되고, 최신 목록 `candidates/index.json`은 재생성 가능하다. 분류(발굴 후보 found / 자료 부족·근거 보강 필요 needs_evidence / 추적 추천 recommended)와 추적 상태(이 가설 추적 중 / 기업만 다른 가설로 추적 중 / 종료 / 미추적, 원장 기준)는 따로 둔다. 숫자 자료가 빠지면 '자료 부족', 사람이 `candidate_evidence.json`에 review_status=needs_evidence와 사유를 적으면 '근거 보강 필요'로 표시하며 둘 다 승인을 막고 스크린 통과 사실은 유지한다. 추적 추천은 다섯 설명(이익 경로·지속 근거·시장 기대와의 차이·반증·다음 점검)이 모두 출처와 연결되고(출처마다 제목·제공자·관측일·발표일 또는 null·인용 또는 필드 근거), `candidates.py approve CAN-… --by 이름`이 현재 입력으로 카드를 다시 계산해 화면의 버전과 같을 때만 승인한다. 오래된 결과·자료 부족·근거 보강 필요는 거부하고, 승인에는 승인자·시각·버전·근거 해시·관측 ID를 남기며 알림을 직접 보내지 않는다. 문장과 URL이 있다는 것만으로 실질 검증이 끝났다는 뜻은 아니다. 근거나 수치가 바뀌면 재검토 표시로 돌아간다. 번역 거부 이유는 `translation_rejects.json`에 원문 해시와 함께 남고 다음 일간 실행에서 다시 시도한다. 화면·문서 카드에는 영문 원문을 함께 보여 준다. 목록에서 빠진 후보는 마지막 관측일과 최신 관측으로 확인되는 이유(순위 밖/조건 미충족/자료 없음)를 따로 보여 준다. 회사 한 줄 설명은 Yahoo 프로필을 Gemini로 한국어 번역한 것(원문 해시 캐시, 원문에 없는 숫자가 있으면 거부)이고, 모델 호출은 extract와 하루 예산(max_model_calls)을 공유한다(`model_budget.json`). 최신 스크린 시도가 실패하면 이전 유효 카드에 마지막 유효 관측일과 실패를 표시한다. 결과는 `docs/candidates.md`, 필터·상세 카드 화면 `reports/generated/candidates.html`, 텔레그램 `/screen`(상위 5개)·`/candidate CAN-…`(카드 상세)로 본다. 수동 실행에서 `redo=screen`처럼 적으면(auto 전용) 그 단계와 결과를 쓰는 단계만 오늘 다시 돈다. 텔레그램 `/track CAN-…`는 사용자의 추적 선택을 `promote.promote_candidate`로 원장에 등록한다(뉴스 SIG를 만들지 않음, `origin_candidate_ids` 컬럼에 후보 ID). 같은 기업+가설의 활성 아이디어가 있으면 그 idea_id를 돌려주고 후보 연결만 한 번 기록한다. 새 등록은 관찰·사업 단계 미확인·근거 수준 가설·검토 상태 추적(사용자 선택이며 검증 승격 아님)·사이클 리비전형이며, 등록 당시 후보 버전·원자료·기준 숫자·미확인 항목을 행과 review_history에 남긴다. 활성 아이디어 상한·관측 기한(signal_lookback_days)·식별/통화 확인이 부족하면 쓰기 전에 거절한다. 등록 직후 수집은 하지 않고, 추적 원장 변화가 eps·quarterly·prices·cards의 입력이라 다음 auto 실행에서 새 종목이 수집 대상에 들어간다. entities.json에 없는 티커는 후보 버전이 검증한 거래소·통화로 수집한다. 원장 컬럼이 바뀌면 migrate_v2가 기존 파일을 `data/archive/pre_columns/`에 먼저 보관한다.

일간 `candidate_context.py`(context 단계, screen 뒤·cards 앞)는 카드 대상 후보(A·B 합집합)의 최근 공식 발표를 SEC에서 찾아 한국어 초안을 만든다. 발행사는 스크린 원자료의 CIK, 없으면 저장된 SEC 공식 목록(`sec_issuers.json.gz`)에서 티커·거래소가 정확히 같을 때만 연결하고, 등록된 식별자와 다르면 보류한다(CIK가 entity_id·CAN-ID를 바꾸지 않음). 최근 120일의 8-K(항목 2.02)·6-K·정기 보고서에서 첨부 문서를 본문으로 판별해 `company_documents/DOC-*.json.gz`(문단·표 위치 ID, 괄호 음수 보존, 2MB 초과는 partial)에 보존한다. SEC 요청은 초당 2건·순차(수집기 공용), 하루 100회·10개 기업·180초, 429/5xx 1회 재시도, 403 반복 시 중단한다. 오래 기다린 후보부터 조사하고, 실패는 24시간·관련 문서 없음은 7일 뒤 다시 본다. 초안은 한 요청에 한 기업, 하루 모델 6회(재시도 포함, 공용 예산 20회 중 candidate_context 몫)이며, 인용이 해당 문단·표에 실제로 있고 숫자가 인용 안에 있으며 전망/실적·상향/하향 표현이 원문과 맞는 문장만 `candidate_context_history/CTX-*.json`에 남긴다. EPS 예상 상향과 발표의 연결은 원문이 직접 말하지 않으면 '미확인'이다. 카드에는 '공식 발표에서 확인한 변화(자동 정리·미검토)'로 최대 3개, 반대 근거·한계, 다음 확인을 보여 주고, 자동 초안은 추천 승인·추적 등록·알림을 만들지 않는다. `candidates.py import-context CAN-…`는 초안을 사람 근거 파일로 복사하되 승인은 복사하지 않고 근거 보강 필요 표시를 붙인다. 승인·철회는 `candidate_review_events/`에 추가만 되는 기록으로 남는다. 모델 요청은 재시도까지 요청마다 예산에서 차감한다(extract 11·cards 3·candidate_context 6). 초안의 숫자·단위·통화는 모델이 쓰지 않는다: 모델은 원문 수치 문구(figures)를 그대로 복사하고 숫자 없는 한국어 설명(note_ko)만 주며, 카드 문장은 검증된 원문 수치로 코드가 조립한다. 수치는 인용 안의 완결된 문구(괄호 음수·통화·million/billion 포함)여야 하고, 가장 가까운 연도·분기가 주장한 기간과 같아야 하며, 기간은 기간 형식(연도·분기·회계연도·'N months ended'·월+일/연도)이어야 한다. GAAP 여부는 원문 표기대로만, 자회사·사업부 문장은 모회사 이익의 핵심 근거로 쓰지 않는다(없으면 insufficient_earnings_context). 자동 초안의 추정치 연결은 unconfirmed·temporal_context만 허용한다. 구버전 검증기(현재 context-check-v4 이전) 초안은 보존하되 카드에 쓰지 않는다. 수치에 가장 가까운 지표 언급이 주장한 지표보다 짧은 이름(예: revenue)이면 그 언급이 주장한 지표 언급(예: total company revenue) 안에 있을 때만 같은 지표로 본다. 모델 429는 재시도하지 않고 모든 구성요소의 요청을 서버가 알려준 시각(없으면 다음 KST 날짜)까지 막는다. 503·네트워크는 최대 1회 재시도, 초안은 기업당 1회 시도이며, 남은 시간이 요청 시간 한도와 재시도 대기를 제한한다. 스크린이 오래됐거나 일부 누락이면(held) 원문·초안 모두 새 요청을 하지 않고, 초안은 현재 후보·같은 대상 회계연도만 만든다. 문서 요청 실패는 '관련 문서 없음'과 구분해 24시간 뒤 다시 보고, 일부만 확보하면 확보분을 쓰고 부분(partial)으로 남긴다. SEC 요청은 리다이렉트를 자동으로 따라가지 않고 hop마다 sec.gov 호스트·횟수·예산을 전송 전에 검사하며, 요청 수는 전송 직전에 저장한다. 발행사 목록이 없으면 SEC 공식 목록만 1회 받아(`candidate_context.py --refresh-issuers`와 같은 경로) 스크린을 다시 돌리지 않는다. `mode=context_verify` 수동 실행은 텔레그램 자격증명 없이 발송 차단(RESEARCH_DISABLE_SEND=1) 상태로 발행사 목록·원문·초안·카드만 만들고, 추적·발송·명령 원장과 일간 완료 기록이 바뀌면 실패한다. `verify_cache_only`는 네트워크 없이 다시 렌더하며 모델·SEC 계수와 기존 관측 파일이 바뀌지 않았는지도 확인한다.
...
새 workflow 설정은 KST 매일 09:17 수집·추출·EPS·알림, 03:23/09:23/15:23/21:23 명령 처리다. 주간 검토 보고는 월요일이다. GitHub 스케줄은 정확한 시각을 보장하지 않는다. 사용자 PC 타이머가 09:17에 mode=auto, 03:23·15:23·21:23에 mode=commands로 dispatch하고(09:23 명령은 09:17 auto의 명령 단계가 맡는다) GitHub 예약은 대체용이다(일간 예약=auto, 6시간 예약=commands). PC가 꺼진 날은 대체 예약만 남으며 9/26 실측 지연은 3~5시간이다([J2 진단](docs/j_timer_diagnosis_2026-09-27.md)).
...
초안 오프라인 재검증: `python tests/offline_revalidation.py`는 모델·SEC 요청 없이 실제 출력 fixture와 저장된 초안의 거부 문장을 현재 검증기로 다시 대조해 표로 출력한다(운영 자료 읽기 전용). 결과: [2026-09-27 표](docs/i3_offline_revalidation_2026-09-27.md).
