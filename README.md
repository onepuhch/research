...
신규 후보 알림은 `candidate_alerts.py`(alerts 단계, cards 성공이 필수, extract가 다시 끝나면 재실행)가 보낸다. 뉴스 A/B 신호와 스크린 후보가 KST 하루 한도(daily_candidate_limit=3)를 함께 쓰며, notify.py가 오늘 이미 보낸 뉴스 후보도 한도에 센다. 순서는 사람이 승인한 추적 추천 → 뉴스·스크린 교대(날짜마다 첫 채널 교대, 빈 채널의 칸은 다른 채널이 사용)이며 발송 칸 배분일 뿐 투자 점수가 아니다. 논리 키 기업|가설|이벤트로 한 번만 알리고(순위·수집일·회계연도 이동으로 재알림 없음), 같은 날 같은 기업은 한 번만 알린다. 추적 추천 승인 알림은 승인 버전마다 한 번이다. 스크린 후보는 최신 수집이 완전(success, stale 아님)하고 카드 자료가 모두 있을 때만 알린다. 발송 원장 `candidate_alerts.json`에 예약→결과(sent+message_id / failed / uncertain)를 남긴다. 예약·uncertain도 한도에 포함하고, 확정 실패만 같은 이벤트로 재시도하며, 응답을 잃은 uncertain은 자동 재발송하지 않는다. 첫 도입일에는 스크린 후보를 bootstrap_max(1)개만 보내고 나머지 현재 후보는 초기 관측 목록으로 저장해 이후 새 발견으로 다시 알리지 않는다. notify.py는 추적 종목 위험 알림만 보내며(카드 실패와 무관), 발송 결과를 sent/failed/uncertain으로 구분해 uncertain도 재발송하지 않는다. `--dry-run`은 발송·원장 쓰기가 없다.
