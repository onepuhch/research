...
현재 Yahoo 공개 컨센서스가 자동 수집된다. 수동 보완이 필요하면 이용 권한이 있는 출처에서 동일 회계기간 추정치와 시점을 확인하여 입력한다. 아래는 형식 설명이며 실제 값으로 채운 후 실행한다. 서로 다른 제공자를 하나의 시계열로 합치지 않는다.
...
- FMP 402: 기본 eps_provider=yahoo에서는 해당 API를 호출하지 않는다. 유료 FMP로 전환할 경우에만 접근 권한 확인이 필요하다.
- Yahoo 접근·구조 오류: 상태판에서 실패를 확인하고 제공자 원문과 파서를 점검한다. 다른 회계기준으로 자동 대체하지 않는다.
